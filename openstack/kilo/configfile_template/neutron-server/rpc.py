...
class RpcCallbacks(type_tunnel.TunnelRpcCallbackMixin):
    ...
    def get_mapping_for_port(self, rpc_context, **kwargs):
        """Get port which bond a qos"""
        port_id = kwargs.get('port_id', '')
        plugin = manager.NeutronManager.get_plugin()
        mapping = []
        try:
            mapping = plugin.get_mapping_for_port(rpc_context, port_id)
        except exceptions.NotFound:
            LOG.debug('Port %s mapping not found', port_id)
